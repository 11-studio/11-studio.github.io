...
**Due sole spaziature del maiuscolo**, entrambe in `:root`. Tararle
una per una a occhio, da .08 a .19 secondo il posto, è il gesto che
fa sembrare un sito un esercizio di tipografia invece che un sito.

| Token | Valore | Dove |
|---|---|---|
| `--track-mark` | `.19em` | l'insegna a tutta larghezza |
| `--track-caps` | `.08em` | ogni altra dicitura |
...
**Il marchio è calcolato, non impaginato a occhio.** Corpo, punto di
partenza, ingombro e altezza del riquadro escono da tre misure del
carattere: l'avanzamento delle dieci lettere (6,188 em), le spalle
laterali di F e I (0,076 e 0,077 em), l'altezza delle maiuscole
(0,714 em). Il conto per esteso sta nella sezione MARCHIO A TUTTA
LARGHEZZA di `style.css`.

| | |
|---|---|
| `font-size` | `128.85px` — in `style.css` |
| `viewBox` | `0 0 1000 104` — in nove punti fra le quattro pagine |
| `x` | `-9.79` |
| `textLength` | `1019.71` |

**Cambiando carattere o peso vanno rifatti tutti e quattro**,
altrimenti la parola non tocca più i margini o resta rientrata
rispetto alla riga sotto.
...
**L'insegna a tutta larghezza è un solo componente, usato due volte.**
`FERRAGATTI` scritto è
testo SVG con `textLength` forzato: la parola tocca esattamente i due
margini a qualsiasi larghezza, senza dipendere dal carattere che il
sistema ha davvero. Le misure tengono conto delle spalle laterali,
quindi ad allinearsi al margine è l'inchiostro delle aste, non
l'avanzamento — che è il motivo per cui il marchio sta a piombo con
la riga sotto. Apre il sito in hero e lo chiude nel piede, identico.
Per cambiarlo si modificano entrambi i blocchi `.bigmark`.
