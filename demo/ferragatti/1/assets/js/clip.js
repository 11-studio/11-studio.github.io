...
(function () {
  ...
  var frag = [
    ...
    '// è carta accartocciata. A 0,42 le pieghe larghe comandano e le',
    '// fini restano a fare il pelo: è la stessa forma, senza il',
    '// tritume sopra. Sotto lo 0,35 diventa gomma.',
    'float fbm(vec2 p){',
    '  float v = 0.0, a = 0.5;',
    '  for (int i = 0; i < 5; i++){ v += a * noise(p); p *= 2.02; a *= 0.42; }',
    '  return v;',
    '}',
    '',
    ...
    '// si stendono, che è il modo in cui cade la stoffa. 2,35 sta',
    '// dove il drappo si legge ancora mosso ma non più sbriciolato.',
